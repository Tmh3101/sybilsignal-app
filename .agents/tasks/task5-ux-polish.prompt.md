...
- Chạy lệnh cài đặt thư viện: `npm install sonner` (hoặc `react-hot-toast`). *Sonner được ưu tiên vì dễ custom CSS.*
...
- Component này có nền đen lưới, hiển thị các dòng chữ nhấp nháy (Blink effect) theo thứ tự giả lập:
  ...
  *(Có thể dùng CSS animation cơ bản để hiện dần các dòng chữ này).*
